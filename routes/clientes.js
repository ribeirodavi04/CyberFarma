...
router.post("/", (req, res, next) =>{
    mysql.getConnection((error, conn) =>{
        ...
        conn.query(
            "INSERT INTO clientes (nomeCliente, CPF_Cliente, telefoneCliente, enderecoCliente, data_nasc_Cliente) VALUES(?, ?, ?, ?, ? );",
            [req.body.nome, req.body.cpf, req.body.telefone, req.body.endereco, req.body.dataNasc],
            (error, result, field) =>{
                conn.release()
                if (error){
                    return res.status(500).send({error:error})
        ...
        conn.query(
            `UPDATE clientes
                SET nomeCliente = ?,
                    ...
                    data_nasc_Cliente = ?
                WHERE idCliente = ?;`,
            [req.body.nome, req.body.cpf, req.body.telefone, req.body.endereco, req.body.dataNasc, req.body.idCliente],
            (error, result, field) =>{
                conn.release()
                if (error){
                    return res.status(500).send({error:error})
